...
while(True):
  #login
  print("Welcome to Sanjeev billing system\n"+i)
  ...
  while(True):
    ...
    option = input("Pick the desired option : ").strip()
    if option.lower() == "n":
      break
    ...
    if(option=='1'):
      ...
      for k in crt_prods:
        ...
        sheet["E{}".format(26+count)] = float(k[5])
        sheet["G{}".format(26+count)] = float(k[4])*float(k[5])
      sheet["F10"] = vehi_no
      sheet["E38"] = round(total_quant,3)
      sheet["G38"] = round(total_val)
      sheet["B40"] = Words(round(total_val))+" Only."
      ...
      while(True):
        ...
        for name in bills:
          if(name.split(".")[0]) == billno:
            os.startfile("C:/Users/<PC user name>/Desktop/delivery bills/"+name+'.xlsx')
            break
